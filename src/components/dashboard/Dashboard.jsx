...
import { Search, Upload, X, Eye, Download, ArrowUpDown, ChevronDown, ChevronUp, Users, Phone, CheckCircle, Clock } from 'lucide-react';
        ...
        <div className="activity-timeline-item">
            ...
            <div className="timeline-content-wrapper">
                ...
                <div className="timeline-card">
                    <div className="timeline-card-header" onClick={onToggle} style={{ cursor: 'pointer' }}>
                        <span className="call-by">Call by <span className="font-bold">{log.callBy}</span></span>
...
const Dashboard = () => {
    ...
    const dummyData = [
        ...
        },
        {
            ...
            address: { street: '456 Park Ave', city: 'Mumbai', state: 'MH', zip: '400001' },
            activityLog: [
                {
                    date: '2024-01-13 03:45 PM',
                    callBy: 'Agent Doe',
                    duration: '1m 45s',
                    transcript: "Agent: Hello Ms. Smith, calling to confirm your vehicle details.\nCustomer: Go ahead.\nAgent: Honda City, MH-02-CD-5678?\nCustomer: That's correct."
                }
            ]
        },
        {
        ...
        },
        {
            ...
            address: { street: '321 Hill Rd', city: 'Chennai', state: 'TN', zip: '600001' },
            activityLog: []
        },
        {
            id: 5,
    ...
    return (
        <div className="dashboard-container">
            ...
            <div className="dashboard-header">
                ...
                <button className="action-button upload-button" onClick={() => setIsUploadModalOpen(true)}>
                    <Upload size={20} />
            ...
            <div className="summary-cards">
                ...
                <div className="summary-card">
                    ...
                    <div className="summary-details">
                        ...
                        <span className="summary-value">{dummyData.filter(d => d.status === 'Pending').length}</span>
                    </div>
                </div>
                <div className="summary-card">
                    ...
                    <div className="summary-details">
                        ...
                        <span className="summary-value">{dummyData.filter(d => d.status === 'Done').length}</span>
                    </div>
                </div>
                <div className="summary-card">
                    ...
                    <div className="summary-details">
                        ...
                        <span className="summary-value">{dummyData.reduce((acc, curr) => acc + curr.activityLog.length, 0)}</span>
                    </div>
                </div>
            </div>
                    ...
                    <tbody>
                        {currentData.map((item) => (
                            <tr key={item.id}>
                                ...
                                <td>
                                    <span className={`status-badge ${item.status === 'Done' ? 'status-done' : 'status-pending'}`}>
                                        {item.status}
                                    </span>
                                </td>
                            ...
                            {!selectedFile ? (
                                <label className="upload-area">
                                    ...
                                    <div style={{ marginTop: '1rem' }}>
                                        <p className="upload-text">Click to upload CSV or Excel file</p>
                                        <p className="upload-subtext">Supported formats: .csv, .xlsx</p>
                                    </div>
                                </label>
                            ) : (
                    ...
                    <div className="modal-content" onClick={e => e.stopPropagation()}>
                        <div className="modal-header">
                            <div className="modal-user-header">
                                <div className="avatar modal-avatar">{getInitials(selectedCustomer.name)}</div>
                                <div className="modal-user-info">
                                    <h2>{selectedCustomer.name}</h2>
                                    <span className="modal-user-email">{selectedCustomer.email}</span>
                                </div>
                            </div>
                            ...
                            <div className="details-grid">
                                ...
                                <div className="detail-section">
                                    ...
                                    <div className="detail-row">
                                        ...
                                        <span className="detail-value">{selectedCustomer.vehicleName}</span>
                                    </div>
                                    <div className="detail-row">
                                        <span className="detail-label">Vehicle No</span>
                                        <span className="detail-value">{selectedCustomer.vehicleNo}</span>
                                    </div>
                                    <div className="detail-row">
                                        <span className="detail-label">Status</span>
                                        <span className="detail-value">
                                            <span className={`status-badge ${selectedCustomer.status === 'Done' ? 'status-done' : 'status-pending'}`}>
                                ...
                                <div className="detail-section">
                                    ...
                                    <div className="detail-row">
                                        ...
                                        <span className="detail-value">{selectedCustomer.address.street}</span>
                                    </div>
                                    <div className="detail-row">
                                        <span className="detail-label">City</span>
                                        <span className="detail-value">{selectedCustomer.address.city}</span>
                                    </div>
                                    <div className="detail-row">
                                        <span className="detail-label">State</span>
                                        <span className="detail-value">{selectedCustomer.address.state}</span>
                                    </div>
                                    <div className="detail-row">
                                        <span className="detail-label">Zip Code</span>
                                        <span className="detail-value">{selectedCustomer.address.zip}</span>
                                    </div>
                                </div>
                            </div>
                                    ...
                                    {selectedCustomer.activityLog.length > 0 ? (
                                        selectedCustomer.activityLog.map((log, index) => (
                                            <ActivityItem
                                                ...
                                                onToggle={() => setExpandedActivityId(expandedActivityId === index ? null : index)}
                                            />
                                        ))
                                    ) : (
                                        <p style={{ color: '#94a3b8', fontStyle: 'italic' }}>No activity recorded.</p>
                                    )}
                                </div>
                            </div>
                        </div>
